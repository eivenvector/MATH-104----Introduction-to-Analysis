...
Let $(X,\leq )$ be a preordered set and let $I\subseteq X$.  Then, $I$ is an \emph{interval}\index{Interval} iff for all $x_1,x_2\in I$ with $x_1\leq x_2$, whenever $x_1\leq x\leq x$, it follows that $x\in I$.
\begin{rmk}
...
Let $(X,\leq )$ be a preordered set, let $S\subseteq X$, and let $x\in X$.  Then, $x$ is an \emph{upper-bound}\index{Upper bound} iff $s\leq x$ for all $s\in S$.  $x$ is a \emph{lower-bound}\index{Lower-bound} iff $x\leq s$ for all $s\in S$.
\end{dfn}
\begin{dfn}[Maximum and minimum]
Let $(X,\leq )$ be a preordered set and let $x\in X$.  Then, $x$ is a \emph{maximum}\index{Maximum} of $X$ iff $x$ is an upper-bound of all of $X$.  $x$ is a \emph{minimum}\index{Minimum} of $X$ iff $x$ is a lower-bound of all of $X$.
\end{dfn}
\begin{dfn}[Maximal and minimal]
Let $(X,\leq )$ be a preordered set, let $S\subseteq X$, and let $x\in S$.  Then, $x$ is \emph{maximal}\index{Maximal} in $S$ iff whenever $y\in S$ and $y\geq x$, it follows that $x=y$.  $x$ is \emph{minimal}\index{Minimal} in $S$ iff whenever $y\in S$ and $y\leq x$ it follows that $x=y$.
\begin{rmk}
...
A \emph{semigroup}\index{Semigroup} is a magma $(X,\cdot )$ such that $\cdot$ is associative.
\end{dfn}
\begin{dfn}[Monoid]\label{Monoid}
A \emph{monoid}\index{Monoid} $(X,\cdot ,1)$ is a semigroup $(X,\cdot )$ equipped with an identity $1\in X$.
\end{dfn}
...
A \emph{group}\index{Group} is a monoid $(X,\cdot ,1)$ equipped with a function $-^{-1}:X\rightarrow X$ so that $x^{-1}$ is an inverse of $x$ for all $x\in X$.
\begin{rmk}
...
Let $X$ and $Y$ be magmas and let $f:X\rightarrow Y$ be a function.  Then, we say that $f$ is a \emph{homomorphism}\index{Homomorphism (of magmas)} iff $f(x_1\cdot x_2)=f(x_1)\cdot f(x_2)$ for all $x_1,x_2\in X$, and furthermore, in the case that both $X$ and $Y$ have identities, $f(1)=1$.
\begin{rmk}
...
\begin{dfn}[Rg]
A \emph{rg}\index{Rg} is a set equipped with two binary operations $(X,+,\cdot )$ such that
\begin{enumerate}
...
A \emph{rng}\index{Rng} is a rg such that $(X,+,0,-)$ is a commutative group, that is, a rg that has additive inverses.
\end{dfn}
...
A \emph{rig}\index{Rig} is a rg such that $(X,\cdot ,1)$ is a monoid, that is, a rg that has a multiplicative identity
\begin{rmk}
...
Let $(X,+,0,-,\cdot ,1)$ be a rig.  Then, either (i) there is some $m\in \Z ^+$ such that $m\cdot 1=0\in X$ or (ii) there is no such $m$.  In the former case, the smallest positive integer such that $m\cdot 1=0\in X$ is the \emph{characteristic}\index{Characteristic (of a rig)}, and in the latter case we say that the \emph{characteristic} is $0$.  We denote the characteristic by $\Char (X)$.
\end{dfn}
...
A rg $(X,+,\cdot ,0)$ is \emph{integral}\index{Integral} iff it has the property that, whenever $x\cdot y=0$, it follows that either $x=0$ or $y=0$.
\begin{rmk}
...
A \emph{Skew-field}\index{Skew-field} is a ring $(X,+,\cdot ,0,-,1$) such that $(X\setminus \{ 0\} ,\cdot ,1,^{-1})$ is a group.  That is, it is a ring such that every nonzero element has a multiplicative inverse.
\begin{rmk}
...
Let $(X,+,\cdot )$ and $(Y,+,\cdot )$ be rgs and let $f:X\rightarrow Y$ be a function.  Then, we say that $f$ is a \emph{homomorphism}\index{Homomorphism (of rgs)} iff $f$ is both a homomorphism (of magmas) from $(X,+)$ to $(Y,+)$ and from $(X,\cdot )$ to $(Y,\cdot )$.  Explicitly, this means that
\begin{equation}
...
\end{equation}
We say that $H$ is an \emph{ideal}\index{Ideal} iff this is well-defined on the quotient set $G/H$.  In this case, $G/H$ is itself a group, the \emph{quotient group}\index{Quotient group} of $G$ modulo $H$.
\begin{rmk}
...
\end{equation}
We say that $S$ is an \emph{ideal}\index{Ideal} iff both of these operations are well-defined.  In this case, $R/S$ is the \emph{quotient rng}\index{Quotient rng} of $R$ modulo $S$.
\end{dfn}

...
\begin{mdf}[Eventually XYZ]\label{EventuallyXYZ}
Let $\Lambda \ni \lambda \mapsto a_\lambda \in \R$ be a net.  Then, $\lambda \mapsto a_\lambda$ is \emph{eventually XYZ}\index{Eventually XYZ} iff there is some $\lambda _0$ such that $\{ \lambda \in \Lambda :\lambda \geq \lambda _0\} \ni \lambda \mapsto x_\lambda$ is XYZ.
\begin{rmk}
...
\begin{dfn}[Strict subnet]\label{StrictSubnet}
...
\end{dfn}
\begin{exm}[Dirichlet Function]\label{DirichletFunction}

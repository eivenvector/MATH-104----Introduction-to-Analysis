...
\begin{savenotes}
Let $S\subseteq \R ^d$, let $f:S\rightarrow \R$, let $x\in \Int (S)$, and let $v^a\in \tangent[\R ^d][x]$.  Then, the \emph{derivative}\index{Derivative} of $f$ at $x$ in the direction $v^a$, $v^a\nabla _af(x)$, is defined by
...
The first topic we focus on is the relation between differentiability and continuity.
...
\begin{exm}[A continuous function that is nowhere-differentiable]
\begin{savenotes}
...
\begin{exm}[A function differentiable at a point and not continuous in a neighborhood of that point]
...
\begin{exm}[The hessian need not be symmetric]
...
And now we come to the results which you are probaby most familiar with from calculus.

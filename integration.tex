...
A \emph{topological measure space}\index{Topological measure space} is a $\sigma$-compact topological space equipped with a regular borel measure.
\end{dfn}
...
\begin{thm}\label{MeasurableTopologicalMeasureSpace}
...
\Step{Show that $[\meas _1\times \meas _2](U)=\meas _{\mathrm{U}}(U)$}
...
\Step{Show that $[\meas _1\times \meas _2](S_1\times S_2)=\meas _1(S_1)\meas _2(S)$ for $\meas (S_i)<\infty$}\label{stpProductMeasure.5}
...
\Step{Show that $\meas _{\mathrm{U}}(K_1\times K_2)=\meas _{\mathrm{K}}(K_1\times K_2)$}
...
\Step{Show that $\meas _1\times \meas _2$ is an outer-measure}
...
\end{split}
\end{equation}
and so all of these inequalities must be equalities.  We now check that it is finite on quasicompact sets.  Let $K\subseteq X_1\times X_2$ be quasicompact.  Define $K_i\coloneqq \pi _i(K)\subseteq X_i$.  This is quasicompact by the \nameref{ExtremeValueTheorem}, and so $\meas _i(K_i)<\infty$.  However, $K\subseteq K_1\times K_2$, and hence $[\meas _1\times \meas _2](K)\leq \meas _1(K_1)\meas _2(K_2)<\infty$.

\Step{Show that $\meas _1\times \meas _2$ is additive on finite disjoint unions of quasicompact rectangles}
...
\end{rmk}
\end{exm}
\begin{exm}[A set that is not measurable]\footnote{Construction adapted from \cite[pg.~407]{Pugh}.}\label{exm5.2.47}
...
\begin{dfn}[Characteristic functions and simple functions]
...
\end{equation}
A \emph{simple function}\index{Simple function} is a finite nonnegative linear combination of characteristic functions.
\begin{rmk}
...
\end{rmk}
\begin{rmk}
If every we say a net of functions in $\Bor (X)$ \emph{converges}, we mean that it converges pointwise for almost-every $x\in X$.  You can even apply \nameref{KelleysConvergenceTheorem} to have this define a topology on $\Bor (X)$, but this isn't terribly useful.  For example, the integral will not be continuous with respect to this topology---see \cref{MonotoneCounterexample}.  For us, it will essentially just be used as concise language---it's easier to say just ``converges'' than ``converges pointwise almost-everywhere'' all the time.  (In fact, the same goes for $\Mor _{\AlE}(X,[-\infty ,\infty ])$.)
\end{rmk}
\begin{rmk}
...
\end{equation}
Note that, as $K_m\subseteq \Gamma _f$, $g_m(x)<f(x)$ and $f(x)\leq h_m(x)$. Furthermore, for each fixed $x\in X$, the sequence $m\mapsto g_m(x)$ is nondecreasing, and so by the \nameref{MonotoneConvergenceTheorem}, we may define $g(x)\coloneqq \lim _mg_m(x)$.  Similarly, we may define $h(x)\coloneqq \lim _mh_m(x)$.
\begin{exr}
...
\item (Lebesgue's Monotone Convergence Theorem)\label{Integral.LebesguesMonotoneConvergenceTheorem}\index{Lebesgue's Monotone Convergence Theorem} whenever $\lambda \mapsto f_\lambda$ is a nondecreasing net of functions then
\begin{equation}
...
Let us define $f_\infty :X\rightarrow [0,\infty ]$ by $f_\infty (x)\coloneqq \lim _\lambda f(x)$.  For almost every $x\in X$, the sequence $\lambda \mapsto f_\lambda (x)$ is nondecreasing, and so by the usual \nameref{MonotoneConvergenceTheorem}, this limit exists in $[0,\infty ]$\footnote{If the net is eventually bounded, the \nameref{MonotoneConvergenceTheorem} tells us it converges in $\R$.  Otherwise, it converges to $\infty$.} for almost every $x$, and so defines a function $f_\infty \in \Mor _{\AlE}(X,[0,\infty ])$.  For all the points where we do not have monotonicity, we may without loss of generality take $f_\infty$ to be infinite there.  
...
\end{proof}
\end{thm}
This result presents the properties of the integral which uniquely characterize it.  Of course, the integral satisfies many other properties as well.
\begin{prp}[Triangle Inequality]\index{Triangle Inequality (integral)}\label{TriangleInequality}
...
\begin{exm}[A monotone sequence of functions which converges to $0$ whose integrals converge to $\infty$]\label{MonotoneCounterexample}
...
\begin{thm}[Dominated Convergence Theorem]\label{DominatedConvergenceTheorem}
...
\end{proof}
\end{thm}
\begin{exm}[A sequence of integrable functions converging to an integrable function on a space of finite measure, but for which the limit of the integral is not the integral of the limit]
...
\end{rmk}
\end{dfn}
\begin{thm}[Carath\'{e}odory-Vitali Theorem]\index{Carath\'{e}odory-Vitali Theorem}\label{CaratheodoryVitaliTheorem}
...
\begin{dfn}[Riemann integrable]\label{RiemannIntegrable}
Let $f:[a,b]\rightarrow \R$.  Then, $f$ is \emph{riemann integrable}\index{Riemann integrable} iff $f$ is bounded and continuous almost-everywhere.  In this case, its \emph{riemann integral}\index{Riemann integral} is defined to be $\int _a^b\dif x\, f(x)$.
\begin{rmk}
This is usually a theorem, but besides I think it's a waste of time to develop the riemann integral when the lebesgue integral does everything the riemann integral does (and much more), I include it as a definition just to say I did it.  Congratulations!  You now know the riemann integral!
\emd{rmk}
\end{dfn}
...
The following is quite a useful result about regular measures.
...
\begin{dfn}[Topological measure]\label{TopologicalMeasure}
...
\item (Topologically-additive)\index{Topologically-additive} if $S,T\subseteq X$ are separated by neighborhoods, then $\meas (S\cup T)=\meas (S)+\meas (T)$.
\end{enumerate}
A topological space equipped with a topological measure is a \emph{topological measure space}\index{Topological measure space}.
\begin{rmk}
...
Let $X$ be a uniform space.  A \emph{uniform measure}\index{Uniform measure} on $X$ is an outer-measure $\meas$ on $X$ such that
\begin{enumerate}
\item $\meas$ is regular; and
\item \label{UniformMeasure.ii}(Uniformly-additive)\index{Uniformly-additive} if $S,T\subseteq X$ are uniformly-separated, then $\meas (S\cup T)=\meas (S)+\meas (T)$.
\end{enumerate}
A uniform space equipped with a uniform measure is a \emph{uniform measure space}\index{Uniform measure space}.
\begin{rmk}
...
\end{proof}
\end{prp}
One important fact about uniform measures is that the open sets are automatically measurable.
...
\end{proof}
\end{prp}
\begin{exm}[A topological measure on a uniform space that is not borel]\footnote{Topological measures on uniform spaces are automatically uniform measures---see the remark in \cref{UniformMeasure}.}\label{exm5.1.43}
...
Let $\coord{X,\Phi}$ be an isogeneous space.  A \emph{isogeneous measure}\index{Isogeneous measure} is a uniform measure for which $\widetilde{\mathcal{B}}_\Phi$ is a uniformly-measurable base.
\begin{rmk}
...
\end{rmk}
\end{exr}
\begin{exm}[An uncountable set of zero measure---the Cantor Set]
\begin{savenotes}
...
\end{savenotes}
\end{exm}
\begin{exm}[A set with empty interior of positive measure---the Cantor-Smith-Volterra Set]
\begin{exr}
...
Let $X$ be an outer-measure space and let $f:X\rightarrow \R _0$ be a function.  Then, $f$ is \emph{measurable}\index{Measurable (function)} iff
...
\Step{Show that $\mathrm{I}$ is subadditive}
...
\end{savenotes}
\end{thm}
This gives us the integral for nonnegative functions.  We now extend the definition of the integral to any (integrable) real-valued function.  Before we do so, we will decompose a function into its positive and negative parts.
\begin{dfn}
...
$f:X\rightarrow \R$ is \emph{integrable}\index{Integrable} iff both $\int \dif \meas (x)\, f_+(x)$ and $\int \dif \meas (x)\, f_-(x)$ are finite, in which case we define\footnote{Note that the first part above was only for $f$ \emph{nonnegative}.  This part is to extend the definition of the integral for all real-values functions.}
\begin{equation}
